...
def do_all_statistics_jobs(img_gen, img_shape, suffix='real'):
    ...
    plots.logplotXY(plt, x, y, 
                    ...
                    outfilename = f'{OUPUT_DIR}/ns_{suffix}(L={L},N={n_samples}).pdf',)
    plt.close()
...
if __name__ == '__main__':
    ...
    if clustering_sample_images:
        ...
        plots.plot_imgs_labels(plt, imgs_real, labels_real, outfilename=f'{OUPUT_DIR}/imgs_real(L={L}).pdf')
        plt.close()
        ...
        if filelist and len(filelist) > 0:
            ...
            plots.plot_imgs_labels(plt, imgs_fake, labels_fake, outfilename=f'{OUPUT_DIR}/imgs_fake(L={L}).pdf')
            plt.close()
