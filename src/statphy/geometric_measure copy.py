...
def clustering(imgs, 
    ...
    for img in imgs:
        ...
        if change_type == 'shuffle':
            ...
            label = index[label]
            area = imgprocessing.sum(mat, label, index=np.arange(label.max() + 1) ).astype(int)
...
def get_measures(
    ...
    """
    n_imgs = img_gen.len()      # the number of images
...
def cluster_number_density(all_mass : np.ndarray, img_size: int, n_imgs: int, 
    ...
    ns = Ns.astype(np.float64) / (img_size * n_imgs)
...
@jit
def bining_xy(X: np.ndarray, Y: np.ndarray, nbins: int = None):
    '''
...
def statistics_of_measures(
    ...
    measure = get_measures(img_gen, img_shape=img_shape, target_spin=1, show_progress=show_progress)
    ...
    stats['m_rg'] = bining_xy(measure['all_mass'], np.sqrt(measure['all_Rs2']), nbins=nbins_for_m_gr)
...
if __name__ == '__main__':
    ...
    stats = statistics_of_measures(img_gen, img1.shape, None, None)
